...
INSERT INTO agents (name, description, category, status, trust_score, badges)
VALUES 
('KYC Validator', 'Verifying user identity for financial onboarding', 'compliance', 'running', 92, '[{"type": "AI_ACT", "verified": true}]'::jsonb),
('Privacy Auditor', 'Scanning outgoing data for PII leakages', 'governance', 'review', 88, '[{"type": "GDPR", "verified": true}]'::jsonb),
('Medical Scribe', 'Transcribing clinician notes with clinical safety checks', 'utility', 'scheduled', 95, '[{"type": "DIGA", "verified": true}]'::jsonb);
